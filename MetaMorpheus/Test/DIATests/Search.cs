...
namespace Test.DIATests
{
    public class Search
    {
        ...
        public static void TestBottomUpDIASearch()
        {
            ...
            searchTask.CommonParameters.PrecursorMassTolerance = new PpmTolerance(10);
            string outputFolder = @"E:\DIA\TestSearch\bottomUp_update\oldData\umpire_try_150";
            if (!Directory.Exists(outputFolder))
            {
                Directory.CreateDirectory(outputFolder);
            }
            var ms1XicConstructor = new NeutralMassXicConstructor(new PpmTolerance(20), 1, 0.5, 3, searchTask.CommonParameters.PrecursorDeconvolutionParameters, 0, 1, new Bspline(2, 150));
            ...
            var umpireGroupingEngine = new UmpirePfGroupingEngine(150, 0.3f, 0.2, 0.5, 10, 0, 15, 150);
